...
from lib.constants import (FORMATTED_PREDICTIONS_JUDE,
...
jpd = PredictionDataset('JUDE',
                        predictions_csv=FORMATTED_PREDICTIONS_JUDE)
vpd = PredictionDataset('VAL',
                        predictions_csv=FORMATTED_PREDICTIONS_VALIDATION,
...
for alpha in alphas:
    ...
    mcp = FNRCoP(jpd, alpha=alpha)
    mcp.calibrate()

    # PREDICT #
    formatted_predictions, run = mcp.predict(vpd, output_dir, validate=True)

    # UNCOMMENT TO RUN REPORTS
    # # Run reports
    # vt = ValidationTrial([run], mcp.class_names)
    # vt.run_reports(output_dir, display_classes=DISPLAY_SUBTYPES_DICT)
